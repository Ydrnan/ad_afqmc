...
class TriangularGrid(Lattice):
    ...
    def get_site_coordinate(self, pos):
        """
        Return real-space coordinates of site pos = (x, y) using
        triangular basis a1 = (1, 0), a2 = (1/2, sqrt(3)/2).
        """
        x, y = pos
        a1 = np.array([1.0, 0.0])
        ...
        if self.boundary in ("oxc", "xc"):
            coords = y * a1 + (x // 2) * c + (x % 2) * a2

        elif self.boundary in ("pbc", "obc"):
            coords = y * a1 + x * a2
    ...
    def classify_orbitals_D2_xc(self, tol_e=1e-8, tol_sym=1e-6):
        ...
        vecs_sym = vecs.copy()
        ir_labels = [""] * n_sites
        parities = [(-1, -1) for _ in range(n_sites)]
...
@dataclass
@register_pytree_node_class
class ThreeDimensionalGrid(Lattice):
    l_x: int
    l_y: int
    l_z: int
    shape: tuple | None = None
