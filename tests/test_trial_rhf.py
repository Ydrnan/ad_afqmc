...
@pytest.mark.parametrize("memory_mode", ["low"])
def test_energy_memory_modes_match_high_restricted(memory_mode: RhfMeasMemoryMode):
    ...
    c = _rand_orthonormal(key, norb, nocc).astype(jnp.complex64)
    tr = RhfTrial(mo_coeff=c)
    ctx_high = build_meas_ctx(ham, tr, cfg=RhfMeasCfg(memory_mode="high"))
    ctx_mode = build_meas_ctx(ham, tr, cfg=RhfMeasCfg(memory_mode=memory_mode))

    key, sub = jax.random.split(key)
    a = jax.random.normal(sub, (nocc, nocc)).astype(jnp.complex64)
    a = a + (1.2 + 0.1j) * jnp.eye(nocc, dtype=a.dtype)
    w = c @ a

    e_high = energy_kernel_rw_rh(w, ham, ctx_high, tr)
    e_mode = energy_kernel_rw_rh(w, ham, ctx_mode, tr)
    assert jnp.allclose(e_mode, e_high, rtol=1e-6, atol=1e-6)


@pytest.mark.parametrize("memory_mode", ["low"])
def test_energy_memory_modes_match_high_unrestricted(memory_mode: RhfMeasMemoryMode):
    ...
    key, sub_u, sub_d = jax.random.split(key, 3)
    a_u = jax.random.normal(sub_u, (nocc, nocc)).astype(jnp.complex64)
    a_d = jax.random.normal(sub_d, (nocc, nocc)).astype(jnp.complex64)
    a_u = a_u + (1.3 + 0.1j) * jnp.eye(nocc, dtype=a_u.dtype)
    a_d = a_d + (1.1 - 0.2j) * jnp.eye(nocc, dtype=a_d.dtype)
    wu = c @ a_u
    wd = c @ a_d

    e_high = energy_kernel_uw_rh((wu, wd), ham, ctx_high, tr)
    e_mode = energy_kernel_uw_rh((wu, wd), ham, ctx_mode, tr)
    assert jnp.allclose(e_mode, e_high, rtol=1e-6, atol=1e-6)
...
def test_make_rhf_meas_ops_dispatch_and_kernels_exist():
    ...
    meas_u = make_rhf_meas_ops(sys_u, memory_mode="low")
