...
class UcisdtqTrial:
    """
    Unrestricted CISDTQ trial in an MO basis where the reference
    ...
    Arrays:
      ...
      c2aa: (nocc[0], nvir[0], nocc[0], nvir[0])                      AA doubles
      c2ab: (nocc[0], nvir[0], nocc[1], nvir[1])                      AB doubles
      c2bb: (nocc[1], nvir[1], nocc[1], nvir[1])                      BB doubles
      ...
      c4aaaa: (nocc[0], nvir[0], nocc[0], nvir[0], nocc[0], nvir[0], nocc[0], nvir[0])  AAAA quadruples
      c4aaab: (nocc[0], nvir[0], nocc[0], nvir[0], nocc[0], nvir[0], nocc[1], nvir[1])  AAAB quadruples
      c4aabb: (nocc[0], nvir[0], nocc[0], nvir[0], nocc[1], nvir[1], nocc[1], nvir[1])  AABB quadruples
      c4abbb: (nocc[0], nvir[0], nocc[1], nvir[1], nocc[1], nvir[1], nocc[1], nvir[1])  ABBB quadruples
      c4bbbb: (nocc[1], nvir[1], nocc[1], nvir[1], nocc[1], nvir[1], nocc[1], nvir[1])  BBBB quadruples
    """
...
def get_rdm1(trial_data: UcisdtqTrial) -> jax.Array:
...
def overlap_r(walker: jax.Array, trial_data: UcisdtqTrial) -> jax.Array:
    n_elec_0 = trial_data.nocc[0]
    n_elec_1 = trial_data.nocc[1]
    return overlap_u((walker[:, :n_elec_0], walker[:, :n_elec_1]), trial_data)


def overlap_u(walker: tuple[jax.Array, jax.Array], trial_data: UcisdtqTrial) -> jax.Array:
    wa, wb = walker
...
def make_ucisdtq_trial_ops(sys: System) -> TrialOps:
    ...
    if wk == "generalized":
        raise NotImplementedError("UCISDTQ does not support generalized walkers.")
    raise ValueError(f"unknown walker_kind: {sys.walker_kind}")


def make_ucisdtq_trial_data(data: dict, sys: System) -> UcisdtqTrial:
    return UcisdtqTrial(
        ...
        c4aaaa=jnp.asarray(data["ci4aaaa"]),
        c4aaab=jnp.asarray(data["ci4aaab"]),
        c4aabb=jnp.asarray(data["ci4aabb"]),
        c4abbb=jnp.asarray(data["ci4abbb"]),
        c4bbbb=jnp.asarray(data["ci4bbbb"]),
    )
